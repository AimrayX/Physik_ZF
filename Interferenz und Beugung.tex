...
\textbf{Interferenz:} Überlagerung von Wellen, die statische Intensitätsmuster erzeugt. \\
...
\[
    k \cdot \Delta r - \delta \hspace{3mm} \text{ungradzahliges Vielfaches von $\pi$}
\]
\begin{center}
...
\hspace{5mm} Voraussetzung: gleiche Polarisation \& Wellenlänge \\[1ex]

\subsection{Interferenz an dünnen Schichten}
\textbf{Phasenunterschied:}
\[
    \Delta = \frac{2 \pi}{\lambda'} \cdot 2s - \boldsymbol{\pi} = \frac{2\pi}{\lambda} \cdot 2sn - \boldsymbol{\pi} \leftarrow \text{Phasensprung bei } n_1 < n_2
\]
...
\begin{formulabox}
    \[
        d \cdot \sin{\Theta} = \left(m + \frac{1}{2}\right) \lambda \hspace{5mm} m = 0,  \pm 1, \pm 2, \dots
    \]
\end{formulabox}

\textbf{Intensitätsmaxima} (konstruktiv):

\begin{formulabox}
    \[
        d \cdot \sin{\Theta} = m\lambda \hspace{7mm} m = 0,  \pm 1, \pm 2, \dots
    \]
\end{formulabox}
...
\begin{formulabox}
    \[
        \Rightarrow y_m \approx m \cdot \frac{\lambda l}{d} \hspace{5mm} \text{Position des m-ten Maximums}
    \]
\end{formulabox}
...
Das Gitter besitzt eine grosse Anzahl beleuchteter Spalten $N$. Das nächste Minimum befindet sich dort, wo der Gangunterschied zwischen dem ersten Spalt und dem mittleren Spalt ($N/2$) genau $\frac{\lambda}{2}$ beträgt. (analog: 2. Spalt $\frac{N}{2} + 1$, usw.) \\[1ex]
...
\begin{formulabox}
    \[
        \sin{(\Theta_m)} = m \frac{\lambda}{a} \hspace{7mm} m = \pm 1, \pm 2, \dots
    \]
\end{formulabox}

\subsection{Beugung und Auflösung}
\textbf{Betrachtung:} Beugung an einer kreisrunden Öffnung mit Durchmesser $D$ \\[1ex]
...
Dient als Kriterium für das Auflösungsvermögen eines Instruments \textbf{(Rayleigh Kriterium)}
