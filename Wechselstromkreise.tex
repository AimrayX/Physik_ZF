...
\[
    U_{ind}(t) = \underbrace{U_{max}}_{\propto \ n\cdot |\Vec{B}||\Vec{A}|\cdot \omega} \sin{(\omega t)}
\]
...
\begin{formulabox}
    \[
        P(t) = R\cdot I(t)^2 = R \cdot I_{max}^2 \cdot \cos{(\omega t)}^2
    \]
\end{formulabox}
...
\hspace{5mm} Es gilt:
\[
    U_L(t) = U_{L,max} \cos{( \omega t)} \ \stackrel{!}{=} L \frac{dI}{dt} \ \iff \ \frac{dI}{dt} = \frac{U_{L,max} \cos{( \omega t)}}{L}
\]
...
\[
    L\frac{dI}{dt} + \frac{q}{C} = 0 \ \Rightarrow \ L\frac{d^2q}{dt^2} + \frac{q}{C} = 0 \hspace{2mm} \iff \hspace{2mm} \frac{d^2q}{dt^2} + \frac{1}{LC}q = 0
\]
...
\hspace{3mm} \textbf{Maschenregel:}
\[
    L\frac{dI}{dt} + RI + \frac{q}{C} = 0 \hspace{2mm} \iff \hspace{2mm} L\frac{d^2q}{dt^2} + R\frac{dq}{dt} + \frac{1}{C}q = 0
\]
