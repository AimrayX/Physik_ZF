...
\]\[
    \text{Linienladungsdichte: } \lambda = \frac{dq}{dl}
\]
...
\[
\Rightarrow d\Vec{E} = \frac{1}{4\pi\epsilon_0}\frac{dq}{r^2}\hat{e}_r
\]
\begin{formulabox}
\[
...
\textbf{Aussage:} Die Differenz aus der Anzahl der eine Oberfläche verlassenden und der in die Oberfläche eintretenden Feldlinien ist proportional zu der von der Oberfläche eingeschlossenen \textbf{Gesamtladung}


\textbf{elektrischer Fluss} für eine ebene Fläche mit beliebiger Orientierung:
\[
...
\begin{formulabox}
\[
    \Phi_E: \lim_{ \Delta A_i \to 0} \sum_i \Vec{E}_i \cdot \Delta \Vec{A_i}  = \int_A \Vec{E} \cdot d\Vec{A} 
\]
\end{formulabox}
...
\begin{formulabox}
\[
    \Phi_E = \oint_A \Vec{E} \cdot d\Vec{A} = \oint_A E_n dA = \frac{q_{innen}}{\epsilon_0} \hspace{1cm}  EA = \frac{Q}{\epsilon_0}
\]
\end{formulabox}
...
\begin{formulabox}
    ...
    \]
    \[
        \Vec{E}_r = \frac{1}{4\pi\epsilon_0}\frac{q}{r_k^3}r \hspace{3mm}, (r < r_k)
    \]
\end{formulabox}
...
\end{multicols}
    ...
    \begin{center}
    \includegraphics[width=0.34\linewidth]{Abbildungen Physik ZF/Gausssches_Gesetz_dünne_Kugelschale.png}
    \hspace{5mm}
    \includegraphics[width=0.45\linewidth]{Abbildungen Physik ZF/Gausssches_Gesetz_Vollkugel.png}
...
\begin{center}
Unabhängig von der Form der Oberfläche gilt:
\end{center}
\begin{formulabox}
    \[
...
\begin{formulabox}
    \[
    d\phi = \frac{dE_{pot}}{q_0} = -\Vec{E} \cdot d\Vec{s}
    \]
\end{formulabox}
Die Potenzialänderung bei Verschiebung von a nach b ist:
\begin{formulabox}
    \[
    \Delta\phi = \phi_b - \phi_a = \frac{\Delta E_{el}}{q_0} = - \int_a^b \Vec{E} \cdot d\Vec{s}
    \]
\end{formulabox}
...
\subsection{Das Potenzial eines Punktladungssystems}
...
\\[1ex]
Das $\Vec{E}-Feld$ ist gleich dem Negativen des Gradienten des Potenzials $\phi$
\begin{formulabox}
    \[
...
\begin{formulabox}
    \[  
    \phi = -\int_{\infty}^{r_K} \frac{1}{4\pi\epsilon_0}\frac{q}{r^2}dr -\int_{r_K}^{r_P} 0 \ dr = \frac{1}{4\pi\epsilon_0}\frac{q}{r_K} \hspace{5mm} (r_P < r_K) 
    \]
\end{formulabox}
...
Körper, die sich abstossen, besitzen eine höhere potenzielle Energie, wenn sie näher beieinander sind. \\[1ex]
...
\[
    \Rightarrow \text{Arbeit: } W = q_2 \phi = q_2 \frac{1}{4 \pi \epsilon_0}\frac{q_1}{r} = \frac{1}{4 \pi \epsilon_0}\frac{q_1 \cdot q_2}{r}
\]
\begin{formulabox}
    \[
     E_{el} = \frac{1}{2} \sum_i q_i \phi_i \hspace{3mm} \text{elektrische Energie eines Punktladungsystems}
    \]
\end{formulabox}