...
\begin{itemize}
	\item Es gibt zwei entgegengesetzte magnetische Pole (Nord-/ Südpol)
	\item Gleiche Pole stossen sich ab, entgegengesetzte ziehen sich an
	\item Nord-/ Südpole treten nie isoliert auf
	\item Magnetische Feldlinien zeigen Nordpol $\rightarrow$ Südpol
	\item Sie bilden immer geschlossene Schleifen
\end{itemize}
...
\[
    [B] = 1 \ T = \frac{N/C}{m/s} = \frac{N}{A \cdot m}
\]
\hspace{5mm} Alternativ: $1 \ G \ \text{[Gauss]} = 10^{-4}T$ \\[1ex]
\hspace{5mm} $1 \ G \approx$ Stärke des Magnetfeldes an der Erdoberfläche

\subsection{Kraft auf stromdurchflossenen Leiter}
(Ann: $\Vec{B}$ ist konstant über l, keine Winkeländerung)
\[
    \Vec{F} = \left( q \cdot \Vec{v}_d \times \Vec{B} \right) \left( \frac{n}{V} \right) \cdot A \cdot l
\]
\hspace{5mm} Mit: $I = \left( \frac{n}{V} \right) \cdot q \cdot \Vec{v}_d \cdot A$ folgt: \\[1ex]
...
\subsection{Bewegung einer Punktladung im \texorpdfstring{$\Vec{B}$}{B}-Feld}
...
\subsection{Das auf Leiterschleifen und Magnete ausgeübte Drehmoment}
\[
    |\Vec{F}_1| = |\Vec{F}_2| = I \cdot |\Vec{a} \times \Vec{B}| = I |\Vec{a}||\Vec{B}|
\]
\\[1ex]
\textbf{Drehmoment im Punkt P:}
\begin{formulabox}
    \[
        |\Vec{M}| = |\Vec{F}_2||\Vec{b}|\cdot \sin{\theta} = I\cdot \underbrace{|\Vec{a}||\Vec{b}|}_{|\Vec{A}|} \cdot |\Vec{B}| \cdot \sin{\theta}
    \]
\end{formulabox}
...
\[
    \Vec{\mu} = n \cdot I \cdot \Vec{A} \hspace{5mm} [\mu] = 1 \ A \cdot m^2
\]
